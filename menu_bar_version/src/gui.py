...
APP_BUNDLE = os.environ.get("DAT_APP_BUNDLE", "")
...
def set_login_item_enabled(enabled):
    if not APP_BUNDLE:
        return subprocess.CompletedProcess([], 1, "", "App bundle path is unavailable.")
...
class CheckToggle(tk.Frame):
    ...
    def set_checked(self, checked):
        self.variable.set(bool(checked))
        self.draw()
...
def load_menu_bar_config():
    config = configparser.ConfigParser()
    if os.path.exists(CONFIG_FILE):
        config.read(CONFIG_FILE)
    show_subtitles = config.getboolean("MENU_BAR", "show_subtitles", fallback=True)
    open_at_login = config.getboolean("MENU_BAR", "open_at_login", fallback=False)
    return show_subtitles, open_at_login


def save_menu_bar_config(show_subtitles=None, open_at_login=None):
    config = configparser.ConfigParser()
    if os.path.exists(CONFIG_FILE):
        config.read(CONFIG_FILE)
    ...
    if show_subtitles is not None:
        config["MENU_BAR"]["show_subtitles"] = "1" if show_subtitles else "0"
    if open_at_login is not None:
        config["MENU_BAR"]["open_at_login"] = "1" if open_at_login else "0"
    with open(CONFIG_FILE, "w") as f:
        config.write(f)
...
def gui_main():
    ...
    root.geometry(os.environ.get("DAT_WINDOW_GEOMETRY", "560x520"))
    ...
    show_subtitles, open_at_login = load_menu_bar_config()
    offline_only_var = tk.BooleanVar(value=False)
    show_subtitles_var = tk.BooleanVar(value=show_subtitles)
    open_login_var = tk.BooleanVar(value=open_at_login)
    ...
    subtitle_overlay = {"window": None, "label": None}
    closing = {"value": False}
    ...
    def ensure_subtitle_overlay():
        if subtitle_overlay["window"] is not None:
            return
        screen_width = root.winfo_screenwidth()
        screen_height = root.winfo_screenheight()
        width = min(980, max(520, screen_width - 160))
        height = 96
        x = int((screen_width - width) / 2)
        y = int(screen_height - height - 56)
        ...
        subtitle_overlay["window"] = overlay
        subtitle_overlay["label"] = label

    def show_subtitle(text):
        if closing["value"]:
            return
        if not show_subtitles_var.get():
            return
        value = (text or "").strip()
        if not value:
            return
        ensure_subtitle_overlay()
        subtitle_overlay["label"].configure(text=value)
        subtitle_overlay["window"].deiconify()
        subtitle_overlay["window"].lift()

    def hide_subtitle():
        if subtitle_overlay["window"] is not None:
            subtitle_overlay["window"].withdraw()

    def destroy_subtitle():
        if subtitle_overlay["window"] is not None:
            try:
                subtitle_overlay["window"].destroy()
            except tk.TclError:
                pass
        subtitle_overlay["window"] = None
        subtitle_overlay["label"] = None
    ...
    def toggle_subtitles(enabled):
        save_menu_bar_config(show_subtitles=enabled)
        if not enabled:
            hide_subtitle()

    def toggle_login(enabled):
        result = set_login_item_enabled(enabled)
        if result.returncode != 0:
            open_login_var.set(not enabled)
            login_chk.draw()
            message = result.stderr.strip() or "Unable to update Login Items."
            messagebox.showwarning("Open at Login", message)
            return
        save_menu_bar_config(open_at_login=enabled)

    def quit_menu_bar_app():
        on_close()
        parent_pid = os.environ.get("DAT_MENU_BAR_PARENT_PID", "").strip()
        if parent_pid:
            try:
                root.after(500, lambda: os.kill(int(parent_pid), signal.SIGTERM))
            except Exception:
                pass
    ...
    menu_options_row = tk.Frame(settings_body, bg=COLORS["panel"])
    menu_options_row.grid(row=6, column=0, columnspan=3, sticky="ew", pady=(10, 0))
    menu_options_row.columnconfigure(2, weight=1)

    subtitles_chk = CheckToggle(menu_options_row, "Show subtitles", show_subtitles_var, command=toggle_subtitles)
    subtitles_chk.grid(row=0, column=0, sticky="w", padx=(0, 18))

    login_chk = CheckToggle(menu_options_row, "Open at Login", open_login_var, command=toggle_login)
    login_chk.grid(row=0, column=1, sticky="w")

    btn_quit_menu = make_button(menu_options_row, "Quit", lambda: quit_menu_bar_app(), bg=COLORS["field"], fg=COLORS["text"], border=COLORS["border"], padx=12, pady=5, font_size=12)
    btn_quit_menu.grid(row=0, column=2, sticky="e")
    ...
    def start():
        ...
        show_subtitle("Loading translation models...")
        ...
        def run_start():
            ...
            def ready():
                ...
                show_subtitle(f"Listening: {src_var.get()}")
            safe_after(ready)
    ...
    def stop():
        ...
        def finalize():
            ...
            safe_after(lambda: show_subtitle("Translation stopped"))
            safe_after(lambda: root.after(1200, hide_subtitle))
    ...
    set_controls_locked(False, stop_enabled=False)
    refresh_devices()
    if os.environ.get("DAT_SETTINGS_COLLAPSED", "0").strip() == "1":
        toggle_settings()
